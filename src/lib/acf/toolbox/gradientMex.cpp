...
{
public:
    static ACosTable& getInstance()
    {
        static ACosTable instance; // Guaranteed to be destroyed.
...
private:
    ...
    ACosTable(ACosTable const&) = delete;
    void operator=(ACosTable const&) = delete;
};
    ...
    for (x = 0; x < w; x++)
    {
        ...
        if (O != 0)
        {
            ...
            if (full)
            {
                ...
                for (; y < h - 4; y += 4)
                {
                    STRu(O[y + x * h], ADD(LDu(O[y + x * h]), AND(CMPLT(LDu(Gy[y]), SET(0.f)), SET(PI))));
                }
                for (; y < h; y++)
                {
                    O[y + x * h] += (Gy[y] < 0) * PI;
