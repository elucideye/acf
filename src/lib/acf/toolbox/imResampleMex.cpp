...
template <class T>
void resampleCoef(int ha, int hb, int& n, int*& yas, int*& ybs, T*& wts, int bd[2], int pad = 0)
{
...
#define U(o) C[ya + o] * ywts[y * 4 + o]
                ...
                if (ybd[0] > 4)
                {
                    memset(B0, 0, (hb) * sizeof(T)); // ZERO: This condition requires zero initialization
                    for (; y < hn; y++)
                    {
...
{
    B.create(size, A.depth(), A.channels());

    int ha = A.rows(), hb = B.rows(), wa = A.cols(), wb = B.cols(), d = A.channels();
    std::swap(ha, wa);
    std::swap(hb, wb);
    ...
    switch (A.depth())
    {
        ...
        case CV_8U:
        {
            MatP A1(A.size(), CV_32F, A.channels()), B1(size, CV_32F, A.channels());
