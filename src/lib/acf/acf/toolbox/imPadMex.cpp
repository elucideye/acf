...
void imPad(T* A, T* B, int h, int w, int d, int pt, int pb, int pl, int pr, int flag, T val)
{
    ...
    y = pt > pb ? pt : pb;
    mPad = x > y ? x : y;
    bool useLookup = ((flag == 2 || flag == 3) && (mPad > h || mPad > w)) || (flag == 3 && (ct || cb || cl || cr));
...
// Generate some symbols to keep this building (in transition) and avodi compiler errors
template void imPad(float* A, float* B, int h, int w, int d, int pt, int pb, int pl, int pr, int flag, float val);
template void imPad(double* A, double* B, int h, int w, int d, int pt, int pb, int pl, int pr, int flag, double val);

// B = imPadMex(A,pad,type); see imPad.m for usage details
#ifdef MATLAB_MEX_FILE
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    int *ns, ms[3], nCh, nDims, pt, pb, pl, pr, flag, k;
    double* p;
    void *A, *B;
    mxClassID id;
    double val = 0;
    ...
    {
        mexErrMsgTxt("Input pad must be a double array.");
    }
    ...
    {
        mexErrMsgTxt("Input pad must have 1, 2, or 4 values.");
    }
    ...
    if (!mxGetString(prhs[2], type, 1024))
    {
        ...
        {
            mexErrMsgTxt("Invalid pad value.");
        }
    }
    else
    {
