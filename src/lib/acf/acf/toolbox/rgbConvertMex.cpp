...
void rgb2luv_sse(iT* I, float* J, int n, float nrm)
{
    ...
    while (i < n)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            for (i1 = i; i1 < n1; i1 += 4)
            {
                *(_J++) = ADD(ADD(MUL(*(_pR++), _mr), MUL(*(_pG++), _mg)), MUL(*(_pB++), _mb));
            }
        }
        {
            ...
            for (i1 = i; i1 < n1; i1 += 4)
            {
                ...
                *(_pX++) = MUL(_c1024, _y);
                *(_pY++) = SUB(MUL(MUL(_c52, _x), _z), _cun);
                *(_pZ++) = SUB(MUL(MUL(_c117, _y), _z), _cvn);
            }
        }
        {
            ...
            {
                _l = *(_pL++);
                *_pU = SUB(MUL(_l, *_pU), _cminu);
                _pU++;
                *_pV = SUB(MUL(_l, *_pV), _cminv);
                _pV++;
            }
        }
        i = n1;
...
// Added 4/26/2015 (same as above but use cv::Mat to manage memory)
...
void mexFunction(int nl, mxArray* pl[], int nr, const mxArray* pr[])
{
    ...
    dims = (const int*)mxGetDimensions(pr[0]);
    n = dims[0] * dims[1];
