...
static cv::Mat cvtAnyTo8UC4(const cv::Mat &input)
{
    ...
    switch(input.channels())
    {
        ...
        case 1: cv::cvtColor(input, output, cv::COLOR_GRAY2BGRA); break;
        case 3: cv::cvtColor(input, output, cv::COLOR_RGB2BGRA); break;
        case 4: cv::cvtColor(input, output, cv::COLOR_RGBA2BGRA); break;
#else
        case 1: cv::cvtColor(input, output, cv::COLOR_GRAY2RGBA); break;
        case 3: cv::cvtColor(input, output, cv::COLOR_RGB2RGBA); break;
        case 4: output = input; break;
#endif
        default: throw std::runtime_error("Unsupported foramt");
    }
    return output;
}
